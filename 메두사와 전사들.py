...
if bfs(si, sj, ei, ej) == -1:
    ...
    while True:
        ...
        print('[1] 메두사의 이동 후에 메두사의 위치',si,sj)
        my_print()
        # ========================================================================
        # [2] 메두사의 시선
        # 상, 하, 좌, 우 순서
        ...
        def sight(si, sj, d):
            ...
            for i in range(N):          # 돌리고 나서 메두사 위치 갱신
                for j in range(N):
                    if see[i][j]==9:
                        mi,mj=i,j
        ...
        for idx in range(1, len(p_loc)):
            if not p_loc[idx][0] and p_loc[idx][1]:         # 다시 돌이 된 애들 풀어주고
                p_loc[idx][1]=False
        print("전사의 이동 하고 나서")
        my_print()
